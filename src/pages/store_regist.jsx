...
export const StoreRegist = () => {
  ...
  const { web3, account, orderContract, order_c_address, storeNftContract, store_c_address } =
  ...
  const onClickStore = async () => {
    try {
      ...
      const firstMinting = await storeNftContract.methods.firstMinting(account).call();
