...
export const CustomerPayment = () => {
  ...
  const { web3, account, orderContract, order_c_address, exchangeRate, getExchangeRate } =
  ...
  const onClickOrder = async () => {
    var a = web3.utils.numberToHex(
      (
        ((totalFoodCost + Acustomer.deliveryTip + Acustomer.deliveryFee) * 10 ** 18) /
        exchangeRate
      ).toFixed(0)
    );
    try {
    ...
    <div className='bg-[#F8F8F8]'>
      ...
      <div className='bg-white mt-4 px-5 py-4 category-shadow'>
        ...
        <div className='flex mt-4'>
          <input className='w-4 accent-purple' type='radio' name='radio' onClick={onClickPayment} />
      ...
      <div className='flex flex-col gap-2 bg-white my-4 px-5 py-4 category-shadow'>
        ...
        <div className='flex justify-between items-center'>
          ...
          <div className='text-caption'>{Acustomer.deliveryFee.toLocaleString()}원</div>
        </div>
        <div className='flex justify-between items-center'>
          <div className='font-bold text-caption'>배달팁</div>
          <div className='text-caption'>{Acustomer.deliveryTip.toLocaleString()}원</div>
        </div>
        ...
        <div className='flex justify-between items-center border-t-[1.5px] pt-4 mt-4 border-lightGray'>
          ...
          {payment ? (
            <div className='font-bold text-subtitle'>
              {(totalFoodCost + Acustomer.deliveryFee + Acustomer.deliveryTip).toLocaleString()}원
            </div>
          ) : (
            <div className='font-bold text-subtitle'>
              {`${Number(
